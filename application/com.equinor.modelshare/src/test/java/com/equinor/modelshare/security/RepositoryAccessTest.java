...
public class RepositoryAccessTest {
	...
	private static final Path rootFolder = Paths.get("src/test/resources/security/models/").toAbsolutePath();
	private static RepositoryAccessControl ra;

	@BeforeClass
	public static void setUp() {
		ra = new LocalRepositoryAccessControl(rootFolder);
		...
		supervisors.setGroup(RepositoryAccessControl.SUPERVISOR_GROUP);
	}
	...
	public void testBug_132() throws IOException {
		Path root = Paths.get("src/test/resources/security/bug_132/").toAbsolutePath();
		RepositoryAccessControl rac = new LocalRepositoryAccessControl(root);

		// a@user
		Account user = rac.getAuthorizedAccounts().get(7);
